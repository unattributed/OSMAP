...
wrapper_path="${repo_root}/maint/live/osmap-run-v2-readiness-over-ssh.sh"
tmp_root=$(mktemp -d "${TMPDIR:-/tmp}/osmap-v2-readiness-ssh-test.XXXXXX")
...
run_case() {
  ...
  (
    env \
      ...
      sh "${wrapper_path}" --host "${expected_host}" --local-report "${report_path}" "$@" >/dev/null 2>&1
  )
...
for step_name in \
  ...
  helper-peer-auth \
  request-guardrails \
  mailbox-backend-unavailable; do
  assert_contains "${default_command}" "'${step_name}'"
done
assert_contains "${default_fetch_command}" "cat \"\$HOME\"/"
assert_contains "${default_fetch_command}" "osmap-v2-readiness-report.txt"

subset_output=$(
  run_case \
    subset \
    'subset-v2-report=passed' \
    subset-v2-host \
    --remote-project-root '~/custom-v2' \
    --remote-report '~/custom-v2-report.txt' \
    security-check \
    helper-peer-auth \
    request-guardrails
)
...
assert_contains "${subset_command}" "custom-v2"
assert_contains "${subset_command}" "ksh ./maint/live/osmap-live-validate-v2-readiness.ksh --report \"\$HOME\"/"
assert_contains "${subset_command}" "custom-v2-report.txt"
assert_contains "${subset_command}" "security-check"
assert_contains "${subset_command}" "helper-peer-auth"
assert_contains "${subset_command}" "request-guardrails"
assert_not_contains "${subset_command}" "login-send"
assert_contains "${subset_fetch_command}" "cat \"\$HOME\"/"
assert_contains "${subset_fetch_command}" "custom-v2-report.txt"

login_send_output=$(
  run_case \
    login-send-only \
    'login-send-v2-report=passed' \
    login-send-v2-host \
    --remote-project-root '~/helper-v2' \
    --remote-report '~/helper-v2-report.txt' \
    login-send \
...
assert_contains "${login_send_fetch_command}" "helper-v2-report.txt"

printf '%s\n' "v2 readiness ssh wrapper regression checks passed"
