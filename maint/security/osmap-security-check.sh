...
if [ "$run_cargo_phases" -eq 1 ]; then
	...
	OSMAP_V4_ASSURANCE_REPORT="$TMPDIR/osmap-v4-hostile-assurance-report.json" \
		OSMAP_V4_ASSURANCE_ARCHIVE="$TMPDIR/osmap-v4-hostile-assurance-evidence.tar.gz" \
...
echo "==> validating v2 readiness step-set consistency"
sh maint/security/test-osmap-v2-readiness-step-set.sh
...
echo "==> validating session-surface timestamp freshness"
sh maint/security/test-osmap-session-surface-timestamps.sh

echo "==> validating live auth observability wrapper behavior"
sh maint/security/test-osmap-live-validate-auth-observability.sh

echo "==> validating effective client ip telemetry wrapper behavior"
sh maint/security/test-osmap-live-validate-effective-client-ip-telemetry.sh
...
echo "==> validating V4 security claim matrix gate"
sh maint/security/test-osmap-v4-security-claim-matrix-gate.sh

echo "==> validating V5 boundary carry-forward gate"
sh maint/security/test-osmap-v5-boundary-gate.sh

echo "==> validating V6 retirement readiness gate"
...
echo "==> validating V6 observability wrapper behavior"
