...
require_text "$readme" "## Project status"
require_text "$readme" "operator-integrated software, not a turnkey general-availability release."
require_text "$readme" "V13 | Reviewed production deployment, credentialed WSTG assurance, and adversarial validation closeout"
require_text "$readme" "V14 | Authenticated WebUI modernization, accessibility, and OpenPGP state UX"
require_text "$readme" "V15 | Deployed HTTP edge/origin parser assurance"
require_text "$readme" "the current release evidence is still bounded;"
require_text "$readme" "complete Roundcube replacement, general hostile-email safety, universal"
require_text "$readme" "runtime decrypt, verify, sign, encrypt, PGP/MIME, and key-management"
