...
require_text "$readme" "The current project status summary is maintained in"
require_text "$readme" "\`docs/CURRENT_PROJECT_STATUS.md\`"
require_text "$readme" "The current release evidence is still bounded."
require_text "$readme" "V4 remains the historical tagged"
require_text "$readme" "V9 | Selected-cohort release-candidate gate is PASS"
require_text "$readme" "V12 | OpenPGP secure foundation is complete through Slice 14 closeout readiness, but remains non-cryptographic."
require_text "$readme" "V13 | WSTG assurance integrity and adversarial validation are completed and deployed."
