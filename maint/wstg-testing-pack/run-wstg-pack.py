...
class Runner:
    ...
    def command_injection_host_evidence(self, nonce: str) -> str:
        ...
        command = (
            ...
            "doas grep -F \"$nonce\" /var/log/nginx/osmap.public.access.log 2>/dev/null | tail -40 || true; "
            "printf '%s\\n' '--- nginx error log nonce matches ---'; "
            "doas grep -F \"$nonce\" /var/log/nginx/osmap.public.error.log 2>/dev/null | tail -40 || true; "
