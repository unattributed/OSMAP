#!/bin/sh
#
# Validate bounded HTTP-runtime observability signals on a live OpenBSD host.
#
...
# isolated enforced browser runtime with a one-connection concurrency cap,
# holds one accepted connection open until the read timeout path triggers,
# confirms that a second request receives `503 Service Unavailable`, then
# verifies that normal traffic resumes and that the expected runtime events were
# written to the log.

set -eu

PROJECT_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
WORK_ROOT="${OSMAP_LIVE_WORK_ROOT:-/home/osmap-live-http-observability-$$}"
...
LISTEN_PORT="${OSMAP_LIVE_HTTP_OBSERVABILITY_PORT:-}"
KEEP_WORK_ROOT="${OSMAP_KEEP_WORK_ROOT:-0}"
READ_TIMEOUT_SECS="${OSMAP_HTTP_READ_TIMEOUT_SECS:-5}"
SLOW_HOLD_SECS="${OSMAP_HTTP_OBSERVABILITY_HOLD_SECS:-10}"
...
doas -u _osmap sh -c "
  ...
  exec env \
    ...
    OSMAP_MAILBOX_HELPER_SOCKET_PATH='${HELPER_SOCKET_PATH}' \
    OSMAP_LOG_LEVEL=info \
    OSMAP_HTTP_MAX_CONCURRENT_CONNECTIONS=1 \
    OSMAP_OPENBSD_CONFINEMENT_MODE=enforce \
...
log "occupying the only in-flight slot with a held connection"
(
  sleep "${SLOW_HOLD_SECS}"
) | nc 127.0.0.1 "${LISTEN_PORT}" >/dev/null 2>&1 &
SLOW_PID="$!"

sleep 1

log "confirming over-capacity request is rejected"
SECOND_RESPONSE="$(request_healthz)"
SECOND_STATUS="$(status_line "${SECOND_RESPONSE}")"
[ "${SECOND_STATUS}" = "HTTP/1.1 503 Service Unavailable" ] || {
  log "expected over-capacity request to return 503"
  printf '%s\n' "${SECOND_RESPONSE}"
  exit 1
}

sleep "$((READ_TIMEOUT_SECS + 1))"

log "confirming runtime recovers for normal requests after timeout"
THIRD_RESPONSE="$(request_healthz)"
THIRD_STATUS="$(status_line "${THIRD_RESPONSE}")"
[ "${THIRD_STATUS}" = "HTTP/1.1 200 OK" ] || {
...
log "verifying expected observability events"
doas grep -q 'action=http_connection_capacity_reached' "${LOG_PATH}" || {
  log "missing connection-capacity event"
  doas cat "${LOG_PATH}"
  exit 1
}
doas grep -q 'action=http_connection_rejected_over_capacity' "${LOG_PATH}" || {
  log "missing over-capacity rejection event"
  doas cat "${LOG_PATH}"
  exit 1
}
doas grep -q 'action=http_request_timed_out' "${LOG_PATH}" || {
  log "missing request-timeout event"
  doas cat "${LOG_PATH}"
  exit 1
}
doas grep -q 'action=http_request_completed' "${LOG_PATH}" || {
  log "missing request-completed event"
  doas cat "${LOG_PATH}"
  exit 1
}

log "live HTTP observability validation passed"
log "second request status: ${SECOND_STATUS}"
log "recovered request status: ${THIRD_STATUS}"
