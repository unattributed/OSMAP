#!/bin/sh
#
# Validate the bounded browser message-move throttle on a live OpenBSD host.
#
...
# with a synthetic validated session, injects one disposable message into the
# validation mailbox, performs one accepted `POST /message/move`, then confirms
# that the second matching move is rejected with `429 Too Many Requests` and
# `Retry-After`.

set -eu

PROJECT_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
WORK_ROOT="${OSMAP_LIVE_WORK_ROOT:-/home/osmap-live-move-throttle-$$}"
...
LISTEN_PORT="${OSMAP_LIVE_MOVE_THROTTLE_PORT:-}"
...
USER_AGENT="osmap-live-move-throttle"
THROTTLE_MAX_MOVES="${OSMAP_MESSAGE_MOVE_THROTTLE_MAX_MOVES:-1}"
THROTTLE_REMOTE_MAX_MOVES="${OSMAP_MESSAGE_MOVE_THROTTLE_REMOTE_MAX_MOVES:-2}"
THROTTLE_WINDOW_SECS="${OSMAP_MESSAGE_MOVE_THROTTLE_WINDOW_SECS:-300}"
THROTTLE_LOCKOUT_SECS="${OSMAP_MESSAGE_MOVE_THROTTLE_LOCKOUT_SECS:-900}"
...
DESTINATION_MAILBOX="${OSMAP_MOVE_THROTTLE_DESTINATION_MAILBOX:-Junk}"
...
cleanup_injected_message() {
  if [ -n "${MESSAGE_SUBJECT:-}" ]; then
    doas -u vmail /usr/local/bin/doveadm -o stats_writer_socket_path= \
      ...
      >/dev/null 2>&1 || true
    doas -u vmail /usr/local/bin/doveadm -o stats_writer_socket_path= \
      expunge -u "${VALIDATION_USER}" mailbox "${DESTINATION_MAILBOX}" header Subject "${MESSAGE_SUBJECT}" \
      >/dev/null 2>&1 || true
  fi
}
...
NOW="$(date +%s)"
EXPIRES_AT="$((NOW + 3600))"
MESSAGE_SUBJECT="OSMAP move throttle proof ${NOW}-$$"
...
doas -u _osmap sh -c "
  ...
  exec env \
    ...
    OSMAP_MESSAGE_MOVE_THROTTLE_MAX_MOVES='${THROTTLE_MAX_MOVES}' \
    OSMAP_MESSAGE_MOVE_THROTTLE_REMOTE_MAX_MOVES='${THROTTLE_REMOTE_MAX_MOVES}' \
    OSMAP_MESSAGE_MOVE_THROTTLE_WINDOW_SECS='${THROTTLE_WINDOW_SECS}' \
    OSMAP_MESSAGE_MOVE_THROTTLE_LOCKOUT_SECS='${THROTTLE_LOCKOUT_SECS}' \
    OSMAP_OPENBSD_CONFINEMENT_MODE=enforce \
...
inject_message() {
  {
    printf 'From: OSMAP Move Proof <%s>\n' "${VALIDATION_USER}"
    printf 'To: %s\n' "${VALIDATION_USER}"
    printf 'Subject: %s\n' "${MESSAGE_SUBJECT}"
    printf '\n'
    printf 'move throttle validation\n'
  } | /usr/sbin/sendmail -t
}
...
post_move() {
  ...
  {
    printf 'POST /message/move HTTP/1.1\r\n'
    printf 'Host: 127.0.0.1\r\n'
    printf 'User-Agent: %s\r\n' "${USER_AGENT}"
...
FIRST_BODY="csrf_token=${CSRF_TOKEN}&mailbox=INBOX&uid=${uid}&destination_mailbox=${DESTINATION_MAILBOX}"
SECOND_BODY="${FIRST_BODY}"

log "sending first bounded move request"
FIRST_RESPONSE="$(post_move "${FIRST_BODY}")"
...
[ "${FIRST_STATUS}" = "HTTP/1.1 303 See Other" ] || {
  log "first move did not succeed as expected"
  printf '%s\n' "${FIRST_RESPONSE}"
  exit 1
}
[ "${FIRST_LOCATION}" = "/mailbox?name=INBOX&moved_to=${DESTINATION_MAILBOX}" ] || {
  log "first move redirect was unexpected"
  printf '%s\n' "${FIRST_RESPONSE}"
  exit 1
}

log "sending second move request to trigger throttle"
SECOND_RESPONSE="$(post_move "${SECOND_BODY}")"
...
[ "${SECOND_STATUS}" = "HTTP/1.1 429 Too Many Requests" ] || {
  log "second move did not trigger throttle"
  printf '%s\n' "${SECOND_RESPONSE}"
  exit 1
}
[ -n "${RETRY_AFTER}" ] || {
  log "throttled move response did not include Retry-After"
  printf '%s\n' "${SECOND_RESPONSE}"
  exit 1
}

doas grep -q "message_move_throttle_engaged" "${HTTP_LOG_PATH}" || {
  log "move throttle engage event missing from log"
  doas cat "${HTTP_LOG_PATH}"
  exit 1
}
doas grep -q "message_move_throttled" "${HTTP_LOG_PATH}" || {
  log "move throttled event missing from log"
  doas cat "${HTTP_LOG_PATH}"
  exit 1
}

log "live move throttle validation succeeded"
log "first_status=${FIRST_STATUS}"
log "first_location=${FIRST_LOCATION}"
log "second_status=${SECOND_STATUS}"
log "retry_after=${RETRY_AFTER}"
