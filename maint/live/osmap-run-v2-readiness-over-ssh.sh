#!/bin/sh
#
# Run the authoritative Version 2 readiness proof set on the live OpenBSD host
...
SSH_TARGET="${OSMAP_V2_READINESS_SSH_TARGET:-mail.blackbagsecurity.com}"
REMOTE_PROJECT_ROOT="${OSMAP_V2_READINESS_REMOTE_PROJECT_ROOT:-~/OSMAP}"
...
examples:
  ...
  ./maint/live/osmap-run-v2-readiness-over-ssh.sh login-send helper-peer-auth
  ./maint/live/osmap-run-v2-readiness-over-ssh.sh --local-report ./maint/live/latest-host-v2-readiness-report.txt security-check
EOF
}
...
parse_args() {
  ...
  if [ -z "${LOCAL_REPORT_PATH}" ]; then
    LOCAL_REPORT_PATH="${PWD}/osmap-v2-readiness-report.txt"
  fi
}

build_remote_command() {
  steps_csv="$1"
  remote_runner="ksh ${REMOTE_READINESS_WRAPPER}"
...
log "running remote V2 readiness proof set on ${SSH_TARGET}"
...
log "wrote local v2 readiness report to ${LOCAL_REPORT_PATH}"
