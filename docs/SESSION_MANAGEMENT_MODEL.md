...
- users can revoke the current session, other sessions, or all sessions
- browser-visible session rows include a bounded device label derived from the
  existing user-agent metadata; OSMAP does not create remembered-device cookies
...
- richer device interpretation beyond the current browser and platform label,
  geolocation, or anomaly scoring around the current session metadata
