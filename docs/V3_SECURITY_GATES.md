...
| Reply/forward attachment safety | `docs/V3_REPLY_FORWARD_ATTACHMENT_HANDLING_DESIGN.md` defines the design gate. Tests must prove selected original attachments are helper-fetched, revalidated at send time, included in aggregate limits, and not silently dropped after confirmation. |
