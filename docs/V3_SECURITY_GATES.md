...
| Draft storage boundary | `docs/V3_DRAFT_SAVE_RESUME_DESIGN.md` defines the design gate. Implementation evidence must prove draft ownership isolation, restrictive state paths, bounded draft and attachment sizes, CSRF and same-origin enforcement, expired-session rejection, cleanup behavior, deterministic storage failure handling, send-success cleanup, send-failure preservation, and redacted evidence handling. |
