...
| Draft save and resume | The design gate is `docs/V3_DRAFT_SAVE_RESUME_DESIGN.md`. Authenticated users can save, list, resume, update, send, and delete bounded drafts. Drafts are scoped to the canonical user. Draft state is stored under reviewed OSMAP state paths with restrictive permissions. Draft POST routes are CSRF-bound and same-origin-bound. Sending a draft submits once and deletes the draft only after successful accepted handoff. Expired or revoked sessions cannot access drafts. Tests cover ownership isolation, invalid draft IDs, oversized drafts, attachment limits, stale sessions, storage failures, backend failures, send-success cleanup, and send-failure preservation. |
| Reply and forward attachment handling | The design gate is `docs/V3_REPLY_FORWARD_ATTACHMENT_HANDLING_DESIGN.md`. Reply and forward compose flows show original attachments explicitly. Users can choose which original attachments to include where policy allows it. Included attachments are fetched through the existing helper-backed, bounded attachment path and revalidated at send time. Aggregate attachment count and size limits include both uploaded and original-message attachments. Failures do not silently drop selected attachments after user confirmation. Tests cover reply default behavior, forward default behavior, selected original attachments, stale source messages, oversized aggregate attachments, and helper failures. |
| Richer bounded search | Users can search one mailbox or all visible mailboxes with documented query fields, refinements if implemented, result sorting, empty-state behavior, and bounded result limits. Current supported field refinements are `all`, `subject`, and `from`; current supported sort columns are UID, subject, from, received, flags, and size. Unsupported query syntax or unsupported refinements return deterministic 400-class responses where they affect backend search semantics. All-mailbox search stays limited to browser-visible mailboxes. Search does not expose backend-only mailbox names. Expensive searches have bounded execution and result behavior. Tests cover valid refinements, invalid refinements, sorting, result caps, unknown mailbox rejection, backend-unavailable behavior, and timeout behavior where applicable. |
...
- evidence that the intended host checkout and deployed runtime match the
  assessed commit or tag
